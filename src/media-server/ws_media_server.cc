...
void serve_client(WebSocketServer & server, WebSocketClient & client)
{
  ...
  /* conditions to check if we are ready to send next_vts */
  if (client.video_playback_buf() > WebSocketClient::MAX_BUFFER_S) {
    return;
  }
  if (*client.video_in_flight() != 0 or
      not channel->vready_to_serve(next_vts)) {
    return;
  }

  /* we are about to send next_vts now;
   * but to ensure audio buffer >= video buffer, we send next_ats first if: */
  if (next_ats < next_vts + channel->vduration()) {
    /* conditions to check if we are ready to send next_ats */
    if (*client.audio_in_flight() != 0 or
        not channel->aready_to_serve(next_ats)) {
      return;
    }
